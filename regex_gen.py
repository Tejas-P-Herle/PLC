...
class RegexGen:
    ...
    def find_most_commons(self, list_a, list_b):
        ...
        def longest_list(list_m, list_n):
            # Compare the lengths of the lists and return the longest list
            if len(list_m) == len(list_n):
                return -1
            else:
                return int(len(list_m) < len(list_n))
        ...
        def find(i, j, match):
            ...
            # Get matches for the two cases
            find_a = find(i, j, True)
            find_b = find(i, j, False)

            # Convert the finds to list type
            if find_a == [] or type(find_a[0]) != list:
                find_a = [find_a]
            if find_b == [] or type(find_b[0]) != list:
                find_b = [find_b]

            # Find longest matches
            longest_match = longest_list(find_a[0], find_b[0])

            # If the length of the lists are exam, return both of them as a list
            if not(find_a == [[]] and find_b == [[]]) and longest_match == -1:
                match_a = commons + find_a[0]
                match_b = commons + find_b[0]
                if len(match_a) == len(match_b):
                    result = []
                    for f in [find_a, find_b]:
                        for m in f:
                            result.append(commons + m)

            # Else add common string to the longer match
            else:
                result = []
                f = find_a if longest_match == 0 else find_b
                for m in f:
                    result = commons + m
            
            # If the result has only one single longest match, return only the
            # longest matching list not the list with the longest matching list
            if result != [] and type(result[0]) == list and len(result[0]) == 1:
                result = result[0]
            return result

        
        # Return the longest list
        finds = find(0, 0, True), find(0, 0, False)
        final_i = longest_list(*finds)
        if final_i == -1:
            raise ValueError("Ambiguous longest match")
        final_res = finds[final_i]
        return final_res
    ...
    def compressable(self, curr_ln):
        ...
        def is_compressable(ln):
            ...
            #                     print(diff_list, diff_0, diff_sec, i)
            #                     raise Exception
            #             
