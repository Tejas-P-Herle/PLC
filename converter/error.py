...
class Error:
    ...
    def __init__(self, err_code_cls, err_code_sub_cls, err_code_desc, raise_error=True, **kwargs):
        ...
        for err_sec_cont, err_sec_code in {'err_cls': err_code_cls,
            ...
            err_sec.append(self.get_err_code_sec(err_sec_code, i))
            i += 1
    ...
    def store_err(cls, sec_no, sec):
        ...
        for ln in sec:
            ln_split = ln.split(': ', 1)
            sec_dict[ln_split[0]] = ln_split[1]

    def get_err_code_sec(self, err_sec_code, err_sec_num):
